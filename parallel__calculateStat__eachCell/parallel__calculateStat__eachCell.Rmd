...
N = c(1:nrow(cell.locations))
quantile.step = 0.004

stat = bplapply(N, function(x){
  getStatForOneCell(x, transcr.profile, cell.locations, quantile.step) 
}, BPPARAM=mcparam)
names(stat) = N
stat = bind_rows(stat, ".id" = "cell")

write.table(stat, file = paste0(save.dir, "par_corr_slope_knot__stat.tab"), row.names = F, col.names = T, quote = F, sep = "\t")
